TARGET_NO_BOOTLOADER := true
TARGET_NO_KERNEL := true
TARGET_NO_RECOVERY := true
TARGET_NO_RADIOIMAGE := true

TARGET_ARCH := arm
TARGET_ARCH_VARIANT := armv7-a-neon
TARGET_CPU_VARIANT := cortex-a7
TARGET_CPU_ABI := armeabi-v7a
TARGET_CPU_ABI2 := armeabi
TARGET_CPU_SMP := true

# set GCC version
TARGET_GCC_VERSION_EXP := 4.9

TARGET_BOARD_PLATFORM := bcm2710
TARGET_PROVIDES_INIT_RC := false
TARGET_COMPRESS_MODULE_SYMBOLS := false
TARGET_PRELINK_MODULE := false

TARGET_USERIMAGES_SPARSE_EXT_DISABLED := true
TARGET_USERIMAGES_USE_EXT4 := true

# Define different partition sizes for bigger cards
TARGET_STORAGE_SIZE := 1GB
# ~1GB SD
ifeq ($(TARGET_STORAGE_SIZE), 1GB)
	BOARD_SYSTEMIMAGE_PARTITION_SIZE := 536870912 # 512M
	BOARD_CACHEIMAGE_PARTITION_SIZE := 134217728 # 128M
	BOARD_CACHEIMAGE_FILE_SYSTEM_TYPE := ext4
	BOARD_USERDATAIMAGE_PARTITION_SIZE := 134217728 # 128M
endif
ifeq ($(TARGET_STORAGE_SIZE), 4GB)
	BOARD_SYSTEMIMAGE_PARTITION_SIZE := 1073741824 # 1024M
	BOARD_CACHEIMAGE_PARTITION_SIZE := 268435456 # 256M
	BOARD_CACHEIMAGE_FILE_SYSTEM_TYPE := ext4
	BOARD_USERDATAIMAGE_PARTITION_SIZE := 2147483648 # 2048M
endif
ifeq ($(TARGET_STORAGE_SIZE), 8GB)
	BOARD_SYSTEMIMAGE_PARTITION_SIZE := 2147483648 # 2048M
	BOARD_CACHEIMAGE_PARTITION_SIZE := 536870912 # 512M
	BOARD_CACHEIMAGE_FILE_SYSTEM_TYPE := ext4
	BOARD_USERDATAIMAGE_PARTITION_SIZE := 4294967296 # 4096M
endif
ifeq ($(TARGET_STORAGE_SIZE), 16GB)
	BOARD_SYSTEMIMAGE_PARTITION_SIZE := 2147483648 # 2048M
	BOARD_CACHEIMAGE_PARTITION_SIZE := 536870912 # 512M
	BOARD_CACHEIMAGE_FILE_SYSTEM_TYPE := ext4
	BOARD_USERDATAIMAGE_PARTITION_SIZE := 10485760000 # 10GB
endif
ifeq ($(TARGET_STORAGE_SIZE), 32GB)
	BOARD_SYSTEMIMAGE_PARTITION_SIZE := 2147483648 # 2048M
	BOARD_CACHEIMAGE_PARTITION_SIZE := 536870912 # 512M
	BOARD_CACHEIMAGE_FILE_SYSTEM_TYPE := ext4
	BOARD_USERDATAIMAGE_PARTITION_SIZE := 27262976 # 26GB
endif
ifeq ($(TARGET_STORAGE_SIZE), 64GB)
	BOARD_SYSTEMIMAGE_PARTITION_SIZE := 2147483648 # 2048M
	BOARD_CACHEIMAGE_PARTITION_SIZE := 536870912 # 512M
	BOARD_CACHEIMAGE_FILE_SYSTEM_TYPE := ext4
	BOARD_USERDATAIMAGE_PARTITION_SIZE := 64424509440 # 60GB
endif

BOARD_FLASH_BLOCK_SIZE := 4096
BOARD_MALLOC_ALIGNMENT := 16

BOARD_USES_GENERIC_AUDIO := false
BOARD_USES_USB_AUDIO := false

BOARD_EGL_CFG := device/brcm/rpi3/egl.cfg
BOARD_GPU_DRIVERS := vc4
USE_OPENGL_RENDERER := true
TARGET_USE_PAN_DISPLAY := true

# Wifi
BOARD_WLAN_DEVICE := bcmdhd
BOARD_WPA_SUPPLICANT_PRIVATE_LIB := lib_driver_cmd_bcmdhd
BOARD_HOSTAPD_PRIVATE_LIB   := lib_driver_cmd_bcmdhd
WPA_SUPPLICANT_VERSION := VER_0_8_X
BOARD_WPA_SUPPLICANT_DRIVER := NL80211

# Bluetooth
BOARD_HAVE_BLUETOOTH := true
BOARD_HAVE_BLUETOOTH_BCM := true
BOARD_BLUETOOTH_BDROID_BUILDCFG_INCLUDE_DIR := device/brcm/rpi3/bluetooth
BOARD_CUSTOM_BT_CONFIG := device/brcm/rpi3/bluetooth/vnd_rpi3.txt

USE_CAMERA_STUB := true

BOARD_SEPOLICY_DIRS := \
    device/brcm/rpi3/sepolicy
